...
export function sumOfSquares(samples) {
  ...
  // 8-way unrolling often hits the sweet spot for Instruction-Level Parallelism (ILP)
  // in V8/Node.js engines, noticeably outperforming 4-way unrolling.
  // This breaks loop-carried dependency chains, allowing the CPU/JS engine
  // to maximize execution speed for large TypedArrays.
  for (; i < len8; i += 8) {
    const s0 = samples[i];
